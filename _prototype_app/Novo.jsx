// Novo contrato — 3-entry-point flow

function NovoContrato({ subRoute, nav }) {
  const step = subRoute; // 'upload' | 'ia' | 'modelo' | '' (entry picker)
      ...
      <>
        ...
        <div className="grid grid-3">
          {[
            ...
            <div key={o.id} className="card card-hover" onClick={() => nav(`novo/${o.id}`)}
              style={{padding: 28, display: 'flex', flexDirection: 'column', gap: 14, minHeight: 220}}>
  ...
  if (step === 'upload')  return <FlowUpload nav={nav}/>;
  if (step === 'ia')      return <FlowIA nav={nav}/>;
  if (step === 'modelo')  return <FlowModelo nav={nav}/>;
  return null;
}

function StepsHeader({ steps, current }) {
  return (
    <div className="row" style={{gap: 10, marginBottom: 28, flexWrap: 'wrap'}}>
      {steps.map((s, i) => (
        <React.Fragment key={i}>
          <div className="row" style={{gap: 8,
            color: i === current ? 'var(--text)' : (i < current ? 'var(--accent)' : 'var(--text-dim)')}}>
            <div style={{
...
function FlowUpload({ nav }) {
  const [stage, setStage] = React.useState(0); // 0=upload, 1=fill, 2=review
  return (
    <>
      <div className="page-head">
        <div>
        ...
        <div className="actions"><button className="btn btn-ghost" onClick={() => nav('novo')}><Icon name="arrow-left" size={14}/>Voltar</button></div>
      </div>
      ...
      {stage === 2 && <ReviewStage onBack={() => setStage(1)} onDone={() => nav('historico')}/>}
    </>
  );
}
  ...
  return (
    <div className="card" style={{padding: 32}}>
      <div style={{
        ...
        {!hasFile ? (
          <>
            <div style={{width: 64, height: 64, borderRadius: 16, background: 'var(--accent-soft)', color: 'var(--accent)',
              display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
          ...
          <>
            <div style={{width: 64, height: 64, borderRadius: '50%', background: 'var(--green)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--bg-deep)'}}>
      ...
      <div className="row sp-between" style={{marginTop: 28}}>
        ...
        <button className="btn btn-primary" disabled={!hasFile} onClick={onNext}
...
function FillStage({ onNext, onBack }) {
      ...
      <div className="card">
        ...
        {[
          ...
          <div className="field" key={k}>
            ...
            <input className="input" value={vals[k]} onChange={e => set(k, e.target.value)}/>
          </div>
        ))}
...
function DocPreview({ vals }) {
  const hl = (t) => <span style={{background: 'var(--green-soft)', color: 'var(--green)', padding: '0 4px', borderRadius: 3, fontWeight: 600}}>{t}</span>;
  return (
    <div style={{
      position: 'sticky', top: 20,
      background: '#fafaf6', color: '#1a1a1a',
      padding: '28px 32px', borderRadius: 14,
      fontFamily: 'Georgia, serif', fontSize: 13, lineHeight: 1.55,
      boxShadow: 'var(--shadow-md)',
    }}>
      <div className="row sp-between" style={{marginBottom: 18}}>
        <span className="chip chip-green" style={{fontFamily:'Inter'}}><span className="chip-dot"/>Pré-visualização ao vivo</span>
        <span style={{fontFamily: 'Inter', fontSize: 11, color: '#666'}}>Página 1 de 3</span>
      </div>
...
function ReviewStage({ onBack, onDone }) {
  ...
  return (
    <div className="grid" style={{gridTemplateColumns: '1.5fr 1fr', gap: 20, alignItems: 'start'}}>
      <div className="card" style={{padding: 0}}>
        ...
        <div style={{background: '#fafaf6', color: '#1a1a1a', padding: 30,
          fontFamily: 'Georgia, serif', fontSize: 13, lineHeight: 1.55, minHeight: 400}}>
          <div style={{textAlign: 'center', fontWeight: 700, fontSize: 15, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 16}}>
            Contrato de Prestação de Serviços
          </div>
          ...
          {added && (
            <div style={{marginTop: 10, padding: 10, background: 'rgba(91,111,209,0.1)', border: '1px solid rgba(91,111,209,0.3)', borderRadius: 6}}>
              <p style={{fontWeight:700, margin:'0 0 4px'}}>Cláusula 3ª — Da Confidencialidade <span style={{fontFamily: 'Inter', fontSize: 10, fontWeight: 700, background: 'rgba(91,111,209,0.25)', color: '#5b6fd1', padding: '1px 6px', borderRadius: 3, marginLeft: 6}}>IA</span></p>
      ...
      <div className="col" style={{gap: 16}}>
        <div className="card" style={{background: 'linear-gradient(135deg, var(--accent-soft), transparent)', borderColor: 'var(--accent-glow)'}}>
          ...
          <div className="row" style={{gap: 8}}>
            {!added ? (
              <button className="btn btn-primary btn-sm" onClick={() => setAdded(true)}><Icon name="plus" size={13}/>Adicionar</button>
            ) : (
  ...
  return (
    <>
      ...
      {!generated ? (
          ...
          <div style={{marginTop: 20}}>
            ...
            <div className="row" style={{gap: 8, flexWrap: 'wrap'}}>
              {['NDA bilateral', 'Consultoria recorrente', 'Locação comercial', 'Honorários êxito', 'Termo de acordo'].map(s => (
                <span key={s} className="chip card-hover" style={{padding: '6px 12px', cursor: 'pointer'}} onClick={() => setPrompt(s)}>{s}</span>
              ))}
            </div>
          </div>
        ...
        <ReviewStage onBack={() => setGenerated(false)} onDone={() => nav('historico')}/>
      )}
    </>
  );
}

function FlowModelo({ nav }) {
  return (
    <>
      <div className="page-head">
        <div>
  ...
  return (
    <div className="grid grid-3">
      {MOCK_TEMPLATES.map(t => (
        <div key={t.id} className="card card-hover" onClick={onPick}
...
Object.assign(window, { NovoContrato, TemplateGrid });
