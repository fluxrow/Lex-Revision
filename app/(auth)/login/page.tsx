...
function LoginPageInner() {
  ...
  const initialEmail =
    ...
    (supabaseSetupRequired && previewModeEnabled ? previewAdminEmail : "marina@silvaadv.com");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Lazy import to avoid server/client issues with env vars during build if not set
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 4, padding: 4, background: 'var(--surface-2)', borderRadius: 10, marginBottom: 28 }}>
        <Link href="/login" style={{ flex: 1, textAlign: 'center', padding: '9px 12px', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer', background: 'var(--surface)', color: 'var(--text)', boxShadow: 'var(--shadow-sm)' }}>
          Entrar
        </Link>
        <Link href="/signup" style={{ flex: 1, textAlign: 'center', padding: '9px 12px', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer', background: 'transparent', color: 'var(--text-muted)' }}>
          Ativar acesso
        </Link>
      </div>

      <h2 style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.02em', margin: '0 0 6px' }}>Acesso para clientes ativos</h2>
      <p className="muted" style={{ margin: '0 0 28px', fontSize: 14 }}>Se você já contratou um plano, entre para continuar.</p>
      ...
      {supabaseSetupRequired && previewModeEnabled && (
        <div style={{ color: 'var(--accent)', fontSize: 13, marginBottom: 18, padding: '10px 12px', background: 'var(--accent-soft)', borderRadius: 8, border: '1px solid var(--border)' }}>
          Preview admin liberado neste ambiente para validacao interna. Use o e-mail <strong>{previewAdminEmail}</strong> com a senha operacional definida no Vercel.
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '16px 0', color: 'var(--text-dim)', fontSize: 12 }}>
        <div style={{ flex: 1, height: 1, background: 'var(--border)' }}/>ou entre com e-mail<div style={{ flex: 1, height: 1, background: 'var(--border)' }}/>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="field">
          <label className="field-label">
            Senha <a href="#" style={{ float: 'right', color: 'var(--accent)', fontWeight: 500 }}>Esqueci minha senha</a>
          </label>
        ...
        <button type="submit" className="btn btn-primary btn-lg" style={{ width: '100%', marginTop: 8 }} disabled={loading}>
          {loading ? 'Entrando...' : (
            <>{supabaseSetupRequired && previewModeEnabled ? "Entrar no preview" : "Entrar"} <Icon name="arrow-right" size={16}/></>
          )}
        </button>
      </form>

      <p className="muted" style={{ textAlign: 'center', marginTop: 20, fontSize: 13 }}>
        Ainda não contratou? <Link href="/#precos" style={{color:'var(--accent)', fontWeight: 600}}>Ver planos da LP</Link>
      </p>
    </>
  );
}
